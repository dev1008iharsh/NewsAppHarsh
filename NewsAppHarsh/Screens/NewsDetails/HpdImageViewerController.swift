...
open class HpdImageViewerController: UIViewController {
    
    public let imageInfo      : HpdImageInfo
    open var transitionInfo : HpdTransitionInfo?
    ...
    public init(imageInfo: HpdImageInfo) {
        self.imageInfo = imageInfo
        super.init(nibName: nil, bundle: nil)
    }
    
    public convenience init(imageInfo: HpdImageInfo, transitionInfo: HpdTransitionInfo) {
    ...
    public convenience init(image: UIImage, imageMode: UIView.ContentMode, imageHD: URL?, fromView: UIView?) {
        let imageInfo = HpdImageInfo(image: image, imageMode: HpdImageInfo.ImageMode(rawValue: imageMode.rawValue)!, imageHD: imageHD)
        if let fromView = fromView {
            self.init(imageInfo: imageInfo, transitionInfo: HpdTransitionInfo(fromView: fromView))
        } else {
            self.init(imageInfo: imageInfo)
        }
    }
...
extension HpdImageViewerController: UIViewControllerTransitioningDelegate {
    ...
    public func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return HpdImageViewerTransition(imageInfo: imageInfo, transitionInfo: transitionInfo!, transitionMode: .present)
    }
    
    public func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return HpdImageViewerTransition(imageInfo: imageInfo, transitionInfo: transitionInfo!, transitionMode: .dismiss)
    }
    
}

class HpdImageViewerTransition: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    init(imageInfo: HpdImageInfo, transitionInfo: HpdTransitionInfo, transitionMode: TransitionMode) {
        self.imageInfo = imageInfo
        self.transitionInfo = transitionInfo
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if transitionMode == .present {
            ...
            let imageViewer = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.to) as! HpdImageViewerController
        ...
        if transitionMode == .dismiss {
            ...
            let imageViewer = transitionContext.viewController(forKey: UITransitionContextViewControllerKey.from) as! HpdImageViewerController
